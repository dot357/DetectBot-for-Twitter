...
start_account_friends = tweepy.Cursor(api.friends, start_account_name).items(100)
...
if start_account_detail.protected != True:	    	#FIXED TWEET PROTECT 12.05.2017-20.31||finding protected account is quite tricky tried 3-4 methods nothing worked so i downloaded user data and found Protected = True so i go to that now if account is protected it shows.
	fn = path + "/" + "followers.txt"				#this is the followers path
	txt(fn)
	file = open(fn ,"w")
	for followers in start_account_followers:                    
		file.write(followers.screen_name + "\n")					#i tried making a dir for every follower but it needs space and cpu to run so i will go with txt for every friend and follower also likes 
